...
import { World, setupEnvironment, createObjectData, applyTimeOfDay } from './world.js';
...
import { FriendsUI } from './friendsUI.js';
import { MiniAvatarViewer } from './avatarViewer.js';
import { getPublishedGame } from './publishedGames.js';
...
class GameSession {
  constructor(canvas, avatarConfigProvider) {
    ...
    this.editor = new Editor({ scene: this.scene, camera: this.camera, renderer: this.renderer, world: this.world, orbitCamera: this.orbitCam, env: this.env });
  ...
  _bindInputs() {
    this.canvas.addEventListener('mousedown', (e) => {
      if (isTouchDevice) return; // touch uses its own action buttons, not click-to-lock
  ...
  loadProject(projectData, { startInEditor = false } = {}) {
    ...
    if (this.project.settings.timeOfDay === undefined) this.project.settings.timeOfDay = 12;
    if (this.project.settings.dayLengthMinutes === undefined) this.project.settings.dayLengthMinutes = 20;
    ...
    this.editor.refresh();
    applyTimeOfDay(this.scene, this.env, this.project.settings.timeOfDay);
    this.editor.bindProject(this.project);
  ...
  update(dt) {
    ...
    if (this.project?.settings?.autoTime) {
      const s = this.project.settings;
      const dayLenMin = Math.max(0.1, s.dayLengthMinutes || 20);
      s.timeOfDay = (((s.timeOfDay ?? 12) + (dt / (dayLenMin * 60)) * 24) % 24 + 24) % 24;
      applyTimeOfDay(this.scene, this.env, s.timeOfDay);
      this.editor._liveTickWorldTime?.(s.timeOfDay);
    }
...
function initApp() {
  ...
  function launchProject(id, { edit = false } = {}) {
    const published = getPublishedGame(id);
    const data = published || getProject(id);
